  ...
  class PagesController extends ProjectController {
    ...
    function view_category() {
      if($this->active_category->isLoaded()) {
		...
		//BOF:mod 20121108
		*/
		$this->redirectTo('project_pages', array('project_id' => $this->active_project->getId(), 'category_id' => $this->request->getId('category_id')));
		//EOF:mod 20121108
      } else {
      ...
      if($this->request->isApiCall()) {
		...
		if ($comments_only_mode){
			$this->smarty->assign(array(
				...
				'pagination' => $pagination,
				'_counter' => ($page - 1) * $this->active_page->comments_per_page,
			));
