...
class RepositoryController extends ProjectController {
  ...
  function commit() {
    ...
    if (!is_array($diff)) {
      ...
      if (is_array($diff)) {
        ...
        $this->active_commit->setCreatedBy(new AnonymousUser($this->active_commit->getCreatedByName(),'no-reply@activecollab.com'));
  ...
  function edit() {
    ...
    $this->smarty->assign(array(
      ...
      'repository_data'     => $repository_data,
      'active_repository'   => $this->active_repository,
      'disable_url_and_type'  => instance_of($this->active_repository->getLastCommit(), 'Commit'),
