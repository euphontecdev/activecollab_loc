<?php
  ...
  class Tickets extends ProjectObjects {
    ...
    function findOpenByProjectByNameSort($project, $min_state = STATE_VISIBLE, $min_visibility = VISIBILITY_NORMAL) {
      return ProjectObjects::find(array(
        'conditions' => array('project_id = ? AND type = ? AND state >= ? AND visibility >= ? AND completed_on IS NULL', $project->getId(), 'Ticket', $min_state, $min_visibility),
        'order' => 'name asc', 
      ));
    ...
    function findByMilestone($milestone, $min_state = STATE_VISIBLE, $min_visibility = VISIBILITY_NORMAL) {
      return ProjectObjects::find(array(
		...
		/*
		//EOF:mod 20121203
        'order' => ($milestone->getId()=='676' ? 'name, priority DESC' : 'priority DESC'),
		//BOF:mod 20121203
		*/
		'order' => 'name',
		//EOF:mod 20121203
    ...
    function findOpenByMilestone($milestone, $min_state = STATE_VISIBLE, $min_visibility = VISIBILITY_NORMAL) {
      return ProjectObjects::find(array(
        'conditions' => array('milestone_id = ? AND type = ? AND state >= ? AND visibility >= ? AND completed_on IS NULL', $milestone->getId(), 'Ticket', $min_state, $min_visibility),
		'order' => 'name',
      ));
    }
