  ...
  class WidgetsController extends ApplicationController {
    ...
    function custom_tabs_manager(){
        ...
        if (is_array($tab_info)){
            $query = "select user_custom_tab_id from healingcrystals_user_custom_tabs where user_id='" . $this->logged_user->getId() . "' and tab_index='" . $tab_info['index'] . "'";
            $result = mysql_query($query);
            if (mysql_num_rows($result)){
                if (empty($tab_info['name']) && empty($tab_info['url'])){
                    mysql_query("delete from healingcrystals_user_custom_tabs  where user_id='" . $this->logged_user->getId() . "' and tab_index='" . $tab_info['index'] . "'");
                } else {
                    mysql_query("update healingcrystals_user_custom_tabs set tab_description='" . mysql_real_escape_string($tab_info['name']) . "', tab_link='" . mysql_real_escape_string($tab_info['url']) . "' where user_id='" . $this->logged_user->getId() . "' and tab_index='" . $tab_info['index'] . "'");
                }
            } else {
                if (!empty($tab_info['name']) && !empty($tab_info['url'])){
                    mysql_query("insert into healingcrystals_user_custom_tabs (user_id, tab_index, tab_description, tab_link) values ('" . $this->logged_user->getId() . "', '" . $tab_info['index'] . "', '" . mysql_real_escape_string($tab_info['name']) . "', '" . mysql_real_escape_string($tab_info['url']) . "')");
                }
            }
            mysql_close($link);
            $this->redirectToUrl(assemble_url('goto_home_tab'));
        } else {
            $query = "select tab_description, tab_link from healingcrystals_user_custom_tabs where user_id='" . $this->logged_user->getId() . "' and tab_index='" . $tab_index . "'";
            $result = mysql_query($query);
            if (mysql_num_rows($result)){
                $tab_info = mysql_fetch_assoc($result);
            }
            mysql_close($link);
        }
        $this->smarty->assign(
                array(
                    'tab_index' => $tab_index, 
                    'action_url' => assemble_url('custom_tabs_manager'), 
                    'tab_description' => htmlentities($tab_info['tab_description']), 
                    'tab_link' => htmlentities($tab_info['tab_link']), 
                    ));
    }
    ...
    function image_uploader(){
        ...
        //$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
        //mysql_select_db(DB_NAME);
        if (!empty($content_type) && !empty($stream) && !empty($file_name)){
            $decoded_stream = base64_decode($stream);
            $handle = fopen(str_replace('activecollab', '', ROOT) . 'imageuploader/' . $file_name, 'w');
            fwrite($handle, $decoded_stream);
            fclose($handle);
            //mysql_query("insert into testing (date_added, content) values (now(), '" . $content_type . "')");
            //mysql_query("insert into testing (date_added, content) values (now(), '" . $stream . "')");
            //mysql_query("insert into testing (date_added, content) values (now(), '" . $file_name . "')");
  }
        //mysql_close($link);
