<?php
  ...
  class StarredObjects {
    ...
    function findStarredPagesByUser($user_id, $project_id){
    	$resp = array();
	  	$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
	  	mysql_select_db(DB_NAME, $link);
	  	$query = "select a.starred_user_id, a.starred_page_type, b.first_name, b.last_name from 
		  		  healingcrystals_starred_user_pages a 
				  inner join healingcrystals_users b on a.starred_user_id=b.id 
				  where a.starred_by_user_id='" . $user_id . "' 
				  order by b.first_name, a.starred_page_type";
		$result = mysql_query($query, $link);
		while($entry = mysql_fetch_assoc($result)){
			$url = '';
			$desc = '';
			switch ($entry['starred_page_type']){
				case 'projects':
					$desc = 'Projects Page: ' . $entry['first_name'] . ' ' . $entry['last_name'];
					$url = assemble_url('project_user_assigned_milestones', array('project_id' => $project_id, 'user_id' => $entry['starred_user_id']));
					break;
				case 'tickets':
					$desc = 'Tickets Page: ' . $entry['first_name'] . ' ' . $entry['last_name'];
					$url = assemble_url('project_user_today_page', array('project_id' => $project_id, 'user_id' => $entry['starred_user_id']));
					break;
			}
			//if (!empty($desc) && !empty($url)){
				$resp[] = array('url'	=> $url, 'desc'	=> $desc);
			//}
		}
	  	mysql_close($link);
	  	return $resp;
