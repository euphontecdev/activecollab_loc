  ...
  class Project extends BaseProject {
    ...
    function getOptions($user) {
      if(!isset($this->options[$user->getId()])) {
        ...
        //BOF: task 04 | AD
          $options->add('manage_milestone_categories', array(
            'url' => $this->getManageMilestoneCategoriesUrl(),
            'text' => lang('Manage Milestone Categories'),
          ));
        //EOF: task 04 | AD
    ...
    function canEdit($user) {
      ...
      $email = $user->getEmail();
      return $user->isProjectLeader($this) || $user->isProjectManager() || $user->isAdministrator() || $email=='kellie@healingcrystals.com';
      //EOF:mod 20110615
    } // canEdit
    ...
    //BOF: task 04 | AD
    function getManageMilestoneCategoriesUrl() {
      return assemble_url('project_manage_milestone_categories', array('project_id' => $this->getId()));
    } // getManageMilestoneCategoriesUrl
  	//EOF: task 04 | AD
  	
  	//BOF: task 01 | AD
    function getUserAllocationsUrl($user) {
      return assemble_url('project_user_view_allocations', array(
        'project_id' => $this->getId(),
        'user_id' => $user->getId(),
      ));
    } // getUserAllocationsUrl
    //EOF: task 01 | AD
    
    function getUserAssignedMilestonesUrl($user) {
      return assemble_url('project_user_assigned_milestones', array(
        'project_id' => $this->getId(),
        'user_id' => $user->getId(),
      ));
    }
    
    function getUserTodayPageUrl($user) {
      return assemble_url('project_user_today_page', array(
        'project_id' => $this->getId(),
        'user_id' => $user->getId(),
      ));
    }
    ...
    function getDepartments(){
    	...
    	$query = "select id, category_name as name from healingcrystals_project_milestone_categories where project_id='" . (int)$this->getId() . "' order by category_name";
    	$result = mysql_query($query, $link);
    	while ($department = mysql_fetch_assoc($result)){
    		$departments[] = array('id' => $department['id'], 
								   'name' => $department['name']);
    	}
    	mysql_close($link);
   		$departments[] = array('id' => '-1', 'name' => 'Uncategorized');
    	return $departments;
    }
