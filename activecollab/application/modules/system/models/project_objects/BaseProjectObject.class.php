<?php
  ...
  class BaseProjectObject extends ApplicationObject {
     ...
     */
    function getBody() {
      return nl2br($this->getFieldValue('body'));
    }
    ...
    function getPriorityName() {
     switch($this->getFieldValue('priority')) {
    ...
    //}
  	...
  	function register_departments($departments, $test = ''){
  		$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
  		mysql_select_db(DB_NAME);
  		//mysql_query("insert into healingcrystals_testing (query, fired_at) value ('" . $test . "', now())");
  		mysql_query("delete from healingcrystals_project_object_categories where object_id='" . $this->getId() . "'", $link);
		foreach($departments as $department_id){
			$department_id = trim($department_id);
			mysql_query("insert into healingcrystals_project_object_categories (object_id, category_id) values ('" . $this->getId() . "', '" . $department_id . "')");
		}
  		mysql_close($link);
  	}
