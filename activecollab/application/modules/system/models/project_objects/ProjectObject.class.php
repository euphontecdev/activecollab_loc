...
/*
 * Change Log
 * Ticket ID #738 - Add AR, FYI & Email to files 12March2012
 ...
 * 7 May 2012 (SA) Ticket #821: modify email reminder for tasks
 */
  /**
  ...
  class ProjectObject extends BaseProjectObject {
    ...
    function getFormattedPriority($language = null) {
      switch($this->getPriority()) {
        case PRIORITY_HOLD:
          return lang('Hold', null, true, $language);
        case PRIORITY_ONGOING:
          return lang('Ongoing', null, true, $language);
     ...
     *
     * @param User $user
     * @param User $to_be_unsubscribed
     * @return boolean
     */
    function canUnsubscribe($user, $to_be_unsubscribed) {
      if(!$this->can_have_subscribers) {
        return false;
      } // if
      ...
      if($this->isSubscribed($to_be_unsubscribed)) {
        return $user->getId() == $to_be_unsubscribed->getId() || $user->isProjectManager($this->getProject()) || $user->isProjectManager();
      } else {
        return false;
      } // if
    } // canUnsubscribe
    ...
    function getPrintTasksUrl() {
      $params = array(
        'project_id' => $this->getProjectId(),
        'parent_id' => $this->getId(),
      );
      
      return assemble_url('project_tasks_print', $params);
    } // getPostTaskUrl
    ...
    function getAttachmentsUrl($show_details = false) {
     ...
     //BOF-20120228SA
    function getAttachmentsAllUrl() {
      return assemble_url('attachments_all', array(
        'project_id' => $this->getProjectId(),
        'object_id' => $this->getId(),
      ));
    } 
    function getCommentsAllUrl() {
      return assemble_url('comments_all', array(
        'project_id' => $this->getProjectId(),
        'object_id' => $this->getParentId(),
      ));
    } 
    //EOF-20120228SA
    /**
     ...
     * @return boolean
     */
    function moveToProject($project, $milestone = null) {
    ...
    function canView($user) {    
      ...
      //Ticket ID #362 - modify Private button (SA) 14 March2012 BOF
      if($this->getVisibility() == 0){
      	    $link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
            mysql_select_db(DB_NAME, $link);
            $query = "select count(*) as total from healingcrystals_assignments where object_id = '".$this->getId()."' and user_id = '".$user->getId()."'";
            $result = mysql_query($query);
            $array = mysql_fetch_array($result);            
           if($array['total']>0){
                return true;
            }else{
            	$query2 = "select count(*) as total from healingcrystals_subscriptions where parent_id = '".$this->getId()."' and user_id = '".$user->getId()."'";
            	$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
            	mysql_select_db(DB_NAME, $link);
      ...
      //Ticket ID #362 - modify Private button (SA) 14 March2012 EOF
     ...
     * @return string
     */
    function getPrintCommentUrl($additional_params = null) {
      $params = array(
        'project_id' => $this->getProjectId(),
        $this->object_id_param_name => $this->getId(),
      );
      ...
      return assemble_url($this->print_route_name, $params);
    } // getPrintUrl
    //EOF:mod 20120217
    ...
    function getQuickOptions($user) {
      if(!isset($this->quick_options[$user->getId()])) {
        ...
        if ($this->getType()=='Checklist'){
        	if($this->canEdit($user)) {
	          $options->add('open_closed_tasks', array(
	            'text' => lang('Set All Tasks to Active'),
	            'url' => assemble_url('project_checklist_open_tasks', array('project_id' => $this->getProjectId(), 'checklist_id' => $this->getId())),
	          ));
        	}
        }
    ...
    //BOF:mod
    function getAjaxEditUrl(){
    	return assemble_url('project_task_ajax_edit', array('project_id' => $this->getProjectId(), 'task_id' => $this->getId()));
    }
    function getAjaxChangePriorityUrl(){
		if ($this->getType()=='Task'){
			return assemble_url('project_task_change_priority', array('project_id' => $this->getProjectId(), 'task_id' => $this->getId()));        
		} elseif ($this->getType()=='Ticket'){
			return assemble_url('project_ticket_change_priority', array('project_id' => $this->getProjectId(), 'ticket_id' => $this->getTicketId()));        
		}
    }
    //EOF:mod
    ...
    function get_auto_email_value(){
             $link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
             mysql_select_db(DB_NAME);
             $query = "select auto_email_status from healingcrystals_project_object_misc where object_id='" .$this->getId() . "'";
             $result = mysql_query($query, $link);
             $array = mysql_fetch_array($result);
             mysql_close($link);
             if($array['auto_email_status']=='1'){
               ...
               if(!empty($users)) 
					return 'Email: '.implode(',',$users);
                else 
					return 'Email: '.$this->getCreatedByName();
             }else{
                 return '';
             }             
        }
    function getActionRequestUsers(){
    	$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
    	mysql_select_db(DB_NAME);
    	$query = "select flag_fyi, flag_actionrequest, email_flag, concat_ws(' ', first_name, last_name) as name from healingcrystals_assignments_flag_fyi_actionrequest a, healingcrystals_users b where b.id = a.user_id and object_id='" .$this->getId() . "'";
    	$result = mysql_query($query, $link);
    	$actionRequestUsers = array();
    	$fyiUsers = array();
    	$emailUsers = array();
    	while($array = mysql_fetch_array($result)){
    		if($array['flag_actionrequest']==1){
    			$actionRequestUsers[] = $array['name'];
    		}
    		if($array['flag_fyi']==1){
    			$fyiUsers[] = $array['name'];
    		}
    		if($array['email_flag']==1){
    			$emailUsers[] = $array['name'];
    		}
    	}
    	mysql_close($link);
    	$string = '';
    	if(!empty($actionRequestUsers))$string .= '<span><b>Action Request</b> marked to: '.implode(', ',$actionRequestUsers).'<br>';
    	if(!empty($fyiUsers))$string .= '<b>FYI</b> marked to: '.implode(', ',$fyiUsers).'<br>';
    	if(!empty($emailUsers))$string .= '<b>Email</b> marked to: '.implode(', ',$emailUsers).'</span>';
    	return $string;
    }
    function getPrintDescriptionUrl($additional_params = null) {
    	$params = array(
    			'project_id' => $this->getProjectId(),
    			'object_id' => $this->getId(),
    	);
    	...
    	return assemble_url('print_description', $params);
    }
	
	function changeMilestoneTo($milestone_id){
		$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
		mysql_select_db(DB_NAME);
		$query = "update healingcrystals_project_objects set milestone_id='" . (int)$milestone_id . "' where id='" . $this->getId() . "'";
		$resp = mysql_query($query, $link);
		mysql_close($link);
		return $resp;
	}
	
	function getDepartmentId($object_id){
		$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
		mysql_select_db(DB_NAME);
	
		$department_id = null;
		$query = mysql_query("select category_id from healingcrystals_project_object_categories where object_id='" . $object_id . "'");
		if (mysql_num_rows($query)){
			$info = mysql_fetch_assoc($query);
			$department_id = $info['category_id'];
		}
	...
	function changeDepartmentTo($department_id, $child_object_types_to_modify = array()){
		$resp = false;
		$link = mysql_connect(DB_HOST, DB_USER, DB_PASS);
		mysql_select_db(DB_NAME);
		mysql_query("delete from healingcrystals_project_object_categories where object_id='" . $this->getId() . "'");
		if (!empty($department_id)){
			$query = "insert ignore into healingcrystals_project_object_categories (object_id, category_id) values ('" . $this->getId() . "', '" . (int)$department_id . "')";
			$resp = mysql_query($query, $link);
		}
		
		if ($this->getType()=='Milestone'){
			foreach($child_object_types_to_modify as $child_object_type){
				$query = "select count(id) as count from healingcrystals_project_objects where type='" . $child_object_type . "' and milestone_id='" . $this->getId() . "'";
				$result = mysql_query($query);
				$info = mysql_fetch_assoc($result);
				if ($info['count']){
					mysql_query("delete from healingcrystals_project_object_categories where object_id in (select a.id from healingcrystals_project_objects a where a.type='" . $child_object_type . "' and a.milestone_id='" . $this->getId() . "')");
					mysql_query("insert into healingcrystals_project_object_categories 
						(select a.id, " . $department_id . " from healingcrystals_project_objects a where a.type='" . $child_object_type . "' and a.milestone_id='" . $this->getId() . "')");
				}
			}
		}
	...
	function convertToPage(&$logged_user, &$error){
		...
		$body = $this->getBody();
		if (empty($body)) $body = 'page Content missing';
		$page->setBody($body);
