<?php
  ...
  class Users extends BaseUsers {
     ...
     * @return array
     */
    function findCompanyManagers($company) {
      $manager_role_ids = array();
      
      $system_roles = Roles::findSystemRoles();
      foreach($system_roles as $system_role) {
        if($system_role->getPermissionValue('manage_company_details')) {
          $manager_role_ids[] = $system_role->getId();
        } // if
      } // foreach
      
      if(count($manager_role_ids) < 1) {
        return null;
      } // if
      
      return Users::find(array(
        'conditions' => array("company_id = ? AND role_id IN (?)", $company->getId(), $manager_role_ids),
        'order' => 'CONCAT(first_name, last_name, email)',
      ));
    } // findCompanyManagers
    
    /**
     * Return number of company managers for a given company
     *
     * @param Company $company
     * @return integer
     */
    function countCompanyManagers($company) {
      $manager_role_ids = array();
      ...
      return Users::count(array("company_id = ? AND role_id IN (?)", $company->getId(), $manager_role_ids));
    ...
    function findForSelect($company = null, $project = null, $exclude_ids = null) {
      ...
      if(instance_of($project, 'Project') && instance_of($company, 'Company')) {
		...
		$users = db_execute_all("SELECT $users_table.id, TRIM(CONCAT($users_table.first_name, ' ', $users_table.last_name)) AS 'display_name', $users_table.email, $users_table.company_id FROM $users_table, $project_users_table WHERE $users_table.id = $project_users_table.user_id AND $users_table.company_id = ? AND $project_users_table.project_id = ? $exclude_filter ORDER BY $users_table.last_name, $users_table.first_name, $users_table.email", $company->getId(), $project->getId());
      } elseif(instance_of($project, 'Project')) {
        //$users = db_execute_all("SELECT $users_table.id, TRIM(CONCAT($users_table.first_name, ' ', $users_table.last_name)) AS 'display_name', $users_table.email, $users_table.company_id FROM $users_table, $project_users_table WHERE $users_table.id = $project_users_table.user_id AND $project_users_table.project_id = ? $exclude_filter ORDER BY display_name, $users_table.email", $project->getId());
		$users = db_execute_all("SELECT $users_table.id, TRIM(CONCAT($users_table.first_name, ' ', $users_table.last_name)) AS 'display_name', $users_table.email, $users_table.company_id FROM $users_table, $project_users_table WHERE $users_table.id = $project_users_table.user_id AND $project_users_table.project_id = ? $exclude_filter ORDER BY $users_table.last_name, $users_table.first_name, $users_table.email", $project->getId());
		...
		$users = db_execute_all("SELECT $users_table.id, TRIM(CONCAT($users_table.first_name, ' ', $users_table.last_name)) AS 'display_name', $users_table.email, $users_table.company_id FROM $users_table WHERE company_id = ? $exclude_filter ORDER BY $users_table.last_name, $users_table.first_name, $users_table.email", $company->getId());
      } else {
        if($exclude_filter) {
		  ...
		  $users = db_execute_all("SELECT $users_table.id, TRIM(CONCAT($users_table.first_name, ' ', $users_table.last_name)) AS 'display_name', $users_table.email, $users_table.company_id FROM $users_table WHERE $users_table.id NOT IN (" . implode(', ', $exclude_ids) . ") ORDER BY $users_table.last_name, $users_table.first_name, $users_table.email");
        } else {
		  ...
		  $users = db_execute_all("SELECT $users_table.id, TRIM(CONCAT($users_table.first_name, ' ', $users_table.last_name)) AS 'display_name', $users_table.email, $users_table.company_id FROM $users_table ORDER BY $users_table.last_name, $users_table.first_name, $users_table.email");
        } // if
      } // if
