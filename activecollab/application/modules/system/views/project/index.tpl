{*}{title}Overview ({$milestones_count} Milestone{if $milestones_count>1}s{/if}, {$tickets_count} Ticket{if $tickets_count>1}s{/if}, {$pages_count} Page{if $pages_count>1}s{/if}, {$tasks_count} Task{if $tasks_count>1}s{/if}, , {$files_count} File{if $files_count>1}s{/if}, {$discussions_count} Discussion{if $discussions_count>1}s{/if}, {$checklists_count} Checklist{if $checklists_count>1}s{/if}){/title}{*}
{title}Overview{/title}
{assign_var name=milestones_url}{assemble route=project_milestones project_id=$active_project->getId()}{/assign_var}
{assign_var name=tickets_url}{assemble route=project_tickets project_id=$active_project->getId()}{/assign_var}
{assign_var name=pages_url}{assemble route=project_pages project_id=$active_project->getId()}{/assign_var}
{assign_var name=files_url}{assemble route=project_files project_id=$active_project->getId()}{/assign_var}
{assign_var name=discussions_url}{assemble route=project_discussions project_id=$active_project->getId()}{/assign_var}
{assign_var name=checklists_url}{assemble route=project_checklists project_id=$active_project->getId()}{/assign_var}
(<a href="{$milestones_url}">{$milestones_count} {lang}Milestone{/lang}{if $milestones_count>1}s{/if}</a>, <a href="{$tickets_url}">{$tickets_count} Ticket{if $tickets_count>1}s{/if}</a>, <a href="{$pages_url}">{$pages_count} Page{if $pages_count>1}s{/if}</a>, {$tasks_count} Task{if $tasks_count>1}s{/if}, <a href="{$files_url}">{$files_count} File{if $files_count>1}s{/if}</a>, <a href="{$discussions_url}">{$discussions_count} Discussion{if $discussions_count>1}s{/if}</a>, <a href="{$checklists_url}">{$checklists_count} Checklist{if $checklists_count>1}s{/if}</a>)
