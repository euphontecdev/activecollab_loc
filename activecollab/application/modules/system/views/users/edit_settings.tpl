  ...
  {form action=$active_user->getEditSettingsUrl() method=post autofocus=no}
    ...
    <fieldset>
      ...
      <div class="section_container">
        ...
        <div>
        {wrap field=generate_daily_mail}
          {label for=userGenerateDailyMail}Generate Daily Mail{/label}
          {yes_no name=user[generate_daily_mail] value=$user_data.generate_daily_mail id=userGenerateDailyMail}  
        {/wrap}
		</div>
      </div>
    </fieldset>
