<?php
   ...
   * @return null
   */
  function incoming_mail_handle_on_prepare_email($tpl, $recipient_email, $context, &$body, &$subject, &$attachments, &$language, $notify_subscribers_string = '') {
    if(instance_of($context, 'ProjectObject') && $context->can_have_comments) {
	  ...
	  if (!empty($notify_subscribers_string)){
		$body =  '<p>' . $notify_subscribers_string . '</p><p>' . $email_splitter . '</p>' . $body;
	  } else {
