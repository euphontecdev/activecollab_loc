...
<div class="form_left_col">
  ...
  {if $active_file->isNew()}
    {wrap field=notify_users}
      ...
      {select_assignees_inline name=notify_users project=$active_project object=$active_file}
    {/wrap}
  {/if}
</div>
...
<div class="form_right_col">
  ...
  {wrap field=category}
	...
	{select_departments name='file[departments][]' object=$active_file project=$active_project}
  {/wrap}
  
{if $logged_user->canSeeMilestones($active_project)}
  {wrap field=milestone_id}
    {label for=fileMilestone required=yes}Milestone{/label}
    {select_milestone name='file[milestone_id]' class="required" value=$file_data.milestone_id project=$active_project id=fileMilestone}
  {/wrap}
{/if}
...
<div class="clear"></div>
{wrap_buttons}
  <button type="button" class="button_add_" id="upload_files"><span><span>{lang}Upload{/lang}</span></span></button></td>
{/wrap_buttons}
{/form}
