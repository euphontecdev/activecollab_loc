{* Change Log *}
{* Ticket ID #738 - Add AR, FYI & Email to files 12 March2012*}
...
<div class="list_view" id="files">
  ...
  {if is_foreachable($files)}
    ...
    {form method="POST" action=$mass_edit_url}
      ...
      <div id="mass_edit">
        ...
        {button id=file_list_submit type=submit}Go{/button}
      </div>
      <!-- MASS EDIT END -->
    {/form}
    <div class="clear"></div>
  ...
  {else}
    ...
    {if instance_of($active_category, 'Category') && $active_category->isLoaded()}
      <p class="empty_page">{lang}There are no files in this category{/lang}. {if $upload_url}<a href="{$upload_url|clean}">{lang}Upload now{/lang}</a>?{/if}</p>
    {else}
      <p class="empty_page">{lang}There are no files to show{/lang}. {if $upload_url}<a href="{$upload_url|clean}">{lang}Upload now{/lang}</a>?{/if}</p>
