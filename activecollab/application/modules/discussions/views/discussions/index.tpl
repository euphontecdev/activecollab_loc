...
<div>
	<div class="list_view">
  		<div class="object_list">
  			<table class="discussions">
    			<tr>
	      		...
	      		<tr>
	      			<td colspan="5"><h2 class="section_name"><span class="section_name_span">{$department.text}</span></h2></td>
	      		</tr>
			      		{/if}
						{if $selected_category_id == '' || $selected_category_id==$department.id}
							{if is_foreachable($department.items)}
								{foreach from=$department.items item=discussion}
				<tr class="discussion {cycle values='odd,even'}" id="discussion{$discussion->getId()}">
					<td class="icon"><a href="{$discussion->getViewUrl()}" class="icon"><img src="{$discussion->getIconUrl($logged_user)}" /></td>
					<td class="name">
						<a href="{$discussion->getViewUrl()}">{$discussion->getName()|clean}</a> <span class="inline_pagination">{pagination pager=$discussion->getPagination(false, $logged_user) sensitive=true}{$discussion->getViewUrl('-PAGE-')}{/pagination}</span>
  		...
  		<ul class="category_list">
  			<li {if $selected_category_id==''}class="selected"{/if}><a href="{$discussions_url}"><span>{lang}View All{/lang}</span></a></li>
	    {foreach from=$departments item=department}
    		<li {if $selected_category_id==$department.id}class="selected"{/if}><a href="{$discussions_url}&category_id={$department.id}"><span>{lang}{$department.text}{/lang}</span></a></li>
    	{/foreach}
