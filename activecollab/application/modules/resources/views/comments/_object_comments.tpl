  ...
  <div class="body">
			  	...
			  	<tr>
			  		...
			  		<td style="font-size:12px;" align="center">
                                            ...
                                            <div id="actionrequest_title" style="display:none;border:1px solid black;background-color:#FBF9EA;width:400px;position:absolute;margin:10px;padding:5px;">
                                                <br/>
                                                Action Requests are a way of<br/>assigning a Task to a Team Member.
                                                {*}<br/><br/>
                                                To set an Action Request,<br/>Select a Priority for the Action Request in the drop-down menu.{*}
                                                <br/>Action Requests that are assigned to a Team Member<br/>
                                                will show up on that Team Members Task Page.
                                                <br/>
                                            </div>
                                        </td>
			  		<td style="font-size:12px;display:none;" align="center">
    ...
    {if is_foreachable($_object_comments_comments)}
      ...
      {if $_total_comments > 20}
            <p id="show_all_ticket_comments_"><span id="visible_count">20</span> of <span id="total_count">{lang total=$_total_comments}:total{/lang}</span> Comments are being displayed - 
			...
			{/if}
			{*}<a href="{$_object_attachments_object->getCommentsAllUrl()}">{lang total=$_total_comments}Show all :total comments{/lang}</a>{*}
			<a href="{$_object_comments_object->getViewUrl()}&show_all=1">{lang total=$_total_comments}Show all :total comments{/lang}</a>
			</p>
    {/if}
        {if $_total_comments <= 20}
...
<input type="hidden" name="reply_to_comment_correction_top_applied" value="0" />
<script type="text/javascript">
  ...
  {if $scroll_to_comment!=''}
	if ($('div[id="comment' + {$scroll_to_comment} + '"]').length==1){ldelim}
		$('html,body').animate({ldelim}
			scrollTop: $('div[id="comment' + {$scroll_to_comment} + '"]').offset().top
		{rdelim});
