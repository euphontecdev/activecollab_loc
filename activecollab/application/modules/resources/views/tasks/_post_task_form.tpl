{form action=$_post_task_form_object->getPostTaskUrl() method=post autofocus=no ask_on_leave=no}
  {wrap field=body}
    ...
    {*}<textarea name="task[body]" class='title required' id="taskSummary" style="width:95%;height:100px;">{$task_data.body}</textarea>{*}
    <textarea name="task[body]" mce_editable="false" class='title required' id="taskSummary">{$task_data.body}</textarea> 
  {/wrap}
