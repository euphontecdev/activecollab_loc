...
<div class="col" style="width:10%;">
  {wrap field=email}
	...
	{*}BOF:mod 20120703{*}
	<input type="checkbox" name="task[email_flag]" {if $task_data.auto_email_status=='1'}checked{/if} value="1" style="width:20px;" onclick="email_reminder_onclick(this);" /> Yes
	{*}EOF:mod 20120703{*}
  {/wrap}
</div>
<div class="col" style="width:15%;">
  {wrap field=daysbeforedue}
	...
	<input type="text" name="task[figure_before_due_date]" style="width:20px;" value="{$task_data.figure_before_due_date}" {if $task_data.auto_email_status!='1'}disabled{/if} />
	<select name="task[unit_before_due_date]" style="width:90px;" {if $task_data.auto_email_status!='1'}disabled{/if}>
		<option value=""></option>
		<option value="D" {if $task_data.unit_before_due_date=='D'}selected{/if}>Day(s)</option>
		<option value="W" {if $task_data.unit_before_due_date=='W'}selected{/if}>Week(s)</option>
		<option value="M" {if $task_data.unit_before_due_date=='M'}selected{/if}>Month(s)</option>
	</select>
  {/wrap}
</div>
<div class="col" style="width:18%;">
  {wrap field=time}
    ...
    <select name="task[reminderhours]" style="width:50px;" {if $task_data.auto_email_status!='1'} disabled{/if}>
        <option value=""></option>
        ...
        {/section}
    </select//--><select name="task[remindermeridian]" style="width:50px;" {if $task_data.auto_email_status!='1'}disabled{/if}>
...
function email_reminder_onclick(chk_ref){ldelim}
	if ($(chk_ref).attr('checked')){ldelim}
		$('input[name="task[figure_before_due_date]"]').removeAttr('disabled');
		$('select[name="task[unit_before_due_date]"]').removeAttr('disabled');
		$('select[name="task[reminderhours]"]').removeAttr('disabled');
		$('select[name="task[remindermeridian]"]').removeAttr('disabled');
	{rdelim} else {ldelim}
		$('input[name="task[figure_before_due_date]"]').attr('disabled', 'disabled');
		$('select[name="task[unit_before_due_date]"]').attr('disabled', 'disabled');
		$('select[name="task[reminderhours]"]').attr('disabled', 'disabled');
		$('select[name="task[remindermeridian]"]').attr('disabled', 'disabled');
	{rdelim}
{rdelim}
</script>