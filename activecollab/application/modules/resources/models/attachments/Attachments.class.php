  ...
  class Attachments extends BaseAttachments {
    ...
    */
    function findByObject($object) {
      ...
      */
      return Attachments::find(array(
        'conditions' => array('( (parent_id = ? AND parent_type = ?) or (parent_id in (select a.id from healingcrystals_project_objects a where a.parent_id=? and a.type="Comment")) )', $object->getId(), get_class($object), $object->getId()),
