<?php
  ...
  function smarty_function_select_date($params, &$smarty) {
    static $counter = 0;
    //$params['value'] = dateval($params['value']);
...
//      $end_date = date('Y/m/d', $end_date->getTimestamp());
    ...
    if($formatted_value) {
      $result .= '<script type="text/javascript">Date.firstDayOfWeek = ' . $first_week_day . '; Date.format = \'mm/dd/yyyy\'; $("#' . $id . '").datePicker().dpSetStartDate("' . $start_date . '").dpSetEndDate("' . $end_date . '").dpSetSelected("' . $formatted_value . '");</script>';
    } else {
      $result .= '<script type="text/javascript">Date.firstDayOfWeek = ' . $first_week_day . '; Date.format = \'mm/dd/yyyy\'; $("#' . $id . '").datePicker().dpSetStartDate("' . $start_date . '").dpSetEndDate("' . $end_date . '");</script>';
    } // if
