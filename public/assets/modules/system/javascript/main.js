    ...
    init_step_1 : function(skip_initial) {
      ...
      if (!step_1_initialized) {
        ...
        step_1.find('.continue').click(function () {
          ...
          //BOF:mod 20111011 #447
          */
         var project_id = project_chooser.find('input[checked=true]').val();
         var object_type = object_chooser.find('input[checked=true]').val()
         var url = '';
         switch(object_type){
           case 'milestone':
             url = App.data.add_milestone_url;
             break;
           case 'checklist':
             url = App.data.add_checklist_url;
             break;
           case 'discussion':
             url = App.data.add_discussion_url;
             break;
           case 'file':
             url = App.data.add_file_url;
             break;
           case 'page':
             url = App.data.add_page_url;
             break;
           case 'ticket':
             url = App.data.add_ticket_url;
             break;
           case 'timerecord':
             url = App.data.add_timerecord_url;
             break;
           default:
             url = '';
         }
         if (url!=''){
           location.href = url.replace('--PROJECT_ID--', project_id);
         }
         //EOF:mod 20111011 #447
          return false;
        });
...
//BOF: mod
function remove_named_anchor_from_url(url){
	var resp = url;	
	
	if (resp.indexOf('?')!=-1){
		var query_string = resp.substring(resp.indexOf('?'));
		if (query_string.indexOf('#')!=-1){
			query_string = query_string.substring(0, query_string.indexOf('#'));
		}
		resp = resp.replace(query_string, '');
		if (resp.indexOf('#')!=-1){
...
function remove_get_param_from_url(url, get_param_to_be_removed){
	try{
		var variable_name = get_param_to_be_removed;
		var query_string = '';
		if (url.indexOf('?')!=-1){
			query_string = url.substring(url.indexOf('?'));
		}
		url = url.replace(query_string, '');
		var amended_query_string = '';
		if (query_string!=''){
			amended_query_string = '?';
			query_string = query_string.substring(1, query_string.length);
			var splitvals = query_string.split('&');
			for(var i=0; i<splitvals.length; i++){
				if (splitvals[i].indexOf(variable_name)==-1){
					amended_query_string += splitvals[i] + '&';
				}
			}
			if (amended_query_string.length>1){
				amended_query_string = amended_query_string.substring(0, amended_query_string.length-1);
			}
		}
		return url + amended_query_string;
	} catch(e){
		alert(e);
	}
}
function page_view_onchange(elemRef){
	try{
	var variable_name = 'page_view=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url +  '&' +variable_name + elemRef.options[elemRef.options.selectedIndex].value;
	} catch(e){ alert(e); }
}

function today_page_oncheckboxclick(elemRef){
	try{
	var variable_name = 'page_view=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url + '&' + variable_name + (elemRef.checked ? '1' : '0');
	} catch(e){ alert(e); }
}

function modify_responsible_status(elemRef){
	try{
	var variable_name = 'is_responsible=';
	var url = remove_named_anchor_from_url(location.href);
	url = remove_get_param_from_url(url, 'page=')
	url = remove_get_param_from_url(url, variable_name)
	location.href = url +  '&' + variable_name + elemRef.options[elemRef.options.selectedIndex].value;
	} catch(e){ alert(e); }
}

function modify_action_request(elemRef){
	try{
	var variable_name = 'action_request_user=';
	var url = remove_named_anchor_from_url(location.href);
	url = remove_get_param_from_url(url, 'page=')
	url = remove_get_param_from_url(url, variable_name)
	location.href = url +  '&' + variable_name + elemRef.options[elemRef.options.selectedIndex].value;
...
function modify_priority_status(elemRef){
	try{
	var msg = '';
	var variable_name = 'priority=';
	var url = remove_named_anchor_from_url(location.href);
	url = remove_get_param_from_url(url, 'page=')
	url = remove_get_param_from_url(url, variable_name)
	location.href = url +  '&' + variable_name + elemRef.options[elemRef.options.selectedIndex].value;
...
function modify_project_association(elemRef){
	try{
	var msg = '';
	var variable_name = 'pID=';
	var url = remove_named_anchor_from_url(location.href);
	url = remove_get_param_from_url(url, 'page=')
	url = remove_get_param_from_url(url, variable_name)
	location.href = url +  '&' + variable_name + elemRef.options[elemRef.options.selectedIndex].value;
...
function modify_department_association(elemRef){
	try{
	var msg = '';
	var variable_name = 'dID=';
	var url = remove_named_anchor_from_url(location.href);
	url = remove_get_param_from_url(url, 'page=')
	url = remove_get_param_from_url(url, variable_name)
	location.href = url +  '&' + variable_name + elemRef.options[elemRef.options.selectedIndex].value;
...
function view_milestones_oncheckboxclick(elemRef){
	try{
	var variable_name = 'page_view=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url + '&' + variable_name + (elemRef.checked ? '1' : '0');
...
function set_pages_view_status(elemRef){
	try{
	var variable_name = 'archived_pages=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url + '&' + variable_name + (elemRef.checked ? '1' : '0');
	} catch(e){ alert(e); }
}

function set_tickets_view_status(elemRef){
	try{
	var variable_name = 'completed_tkts=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url + '&' + variable_name + (elemRef.checked ? '1' : '0');
	} catch(e){ alert(e); }
}

function set_ticket_page(elemRef){
	try{
	var variable_name = 'page=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url +  '&' +variable_name + elemRef.options[elemRef.options.selectedIndex].value;
	} catch(e){ alert(e); }
}

function set_milestone_page(elemRef){
	try{
	var variable_name = 'page=';
	var url = remove_get_param_from_url(location.href, variable_name)
	var variable_name = 'mID=';
	var url = remove_get_param_from_url(url, variable_name)
	location.href = url +  '&' +variable_name + elemRef.options[elemRef.options.selectedIndex].value;
	} catch(e){ alert(e); }
}

function edit_task(buttonRef){
	alert(buttonRef);
}

function view_reminders_oncheckboxclick(elemRef){
	var variable_name = 'flag=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url + '&' + variable_name + (elemRef.checked ? '1' : '0');
}
...
function today_page_onchange(elemRef){
	try{
	var variable_name = 'selected_project=';
	var url = remove_get_param_from_url(location.href, variable_name)
	location.href = url + '&' + variable_name + elemRef.options[elemRef.options.selectedIndex].value;
	} catch(e){ alert(e); }
}
