<?php
   ...
   * @subpackage scripts
   */
  class Upgrade_0019 extends UpgradeScript {
    ...
    function updateUserAgentField() {
      $this->utility->db->execute('ALTER TABLE ' . TABLE_PREFIX . 'user_sessions CHANGE user_agent user_agent text');
    ...
    function updateConfigOptions() {
      if(array_var($this->utility->db->execute_one("SELECT COUNT(*) AS 'row_count' FROM " . TABLE_PREFIX . "modules WHERE name = 'source'"), 'row_count') == 1) {
        $this->utility->db->execute("INSERT INTO " . TABLE_PREFIX . "config_options (name, module, type, value) VALUES ('source_svn_use_output_redirect', 'source', 'system', 'b:0;'), ('source_svn_trust_server_cert', 'source', 'system', 'b:0;')");
      } // if
    ...
    function updateStatusTable() {
      ...
      if(in_array($status_updates_table, $this->utility->db->listTables(TABLE_PREFIX))) {
        $updates = array(
        ...
        foreach($updates as $update) {
      	  $updated = $this->utility->db->execute($update);
      	  if(is_error($updated)) {
    ...
    function updateInvoicingModule() {
      ...
      if(in_array($invoices_table, $this->utility->db->listTables(TABLE_PREFIX))) {
        $updates = array(
        ...
        foreach($updates as $update) {
          $update_table = $this->utility->db->execute($update);
        ...
        $invoices = $this->utility->db->execute("SELECT id,company_id FROM $invoices_table");
        if (is_foreachable($invoices)) {
          ...
          $raw_companies = $this->utility->db->execute("SELECT id,name FROM $companies_table WHERE id IN (?)", array($company_ids));
          $companies = array();
          ...
          foreach ($invoices as $invoice) {
            $result = $this->utility->db->execute("UPDATE $invoices_table SET company_name = ? WHERE id= ?", array(array_var($companies, $invoice['company_id'], 'Unknown Company'), $invoice['id']));
          	if (is_error($result)) {
        ...
        $result = $this->utility->db->execute("ALTER TABLE ".TABLE_PREFIX."invoice_item_templates CHANGE quantity quantity DECIMAL(13,3) UNSIGNED NOT NULL DEFAULT '1', CHANGE unit_cost unit_cost DECIMAL(13,3) NOT NULL DEFAULT '0.00'");
      	if (is_error($result)) {
      		return $result->getMessage();
      	} // if
        $result = $this->utility->db->execute("ALTER TABLE ".TABLE_PREFIX."invoice_payments CHANGE amount amount DECIMAL(13,3) NOT NULL");
      	if (is_error($result)) {
      		return $result->getMessage();
      	} // if
        $result = $this->utility->db->execute("ALTER TABLE ".TABLE_PREFIX."tax_rates CHANGE percentage percentage DECIMAL(6,3) NOT NULL");
      	if (is_error($result)) {
      		return $result->getMessage();
      	} // if
        $result = $this->utility->db->execute("ALTER TABLE ".TABLE_PREFIX."invoice_items CHANGE quantity quantity DECIMAL(13,3) UNSIGNED NOT NULL DEFAULT '1.00', CHANGE unit_cost unit_cost DECIMAL(13,3) NOT NULL DEFAULT '0.00'");
      	if (is_error($result)) {
      		return $result->getMessage();
      	} // if
        $result = $this->utility->db->execute("ALTER TABLE ".TABLE_PREFIX."currencies CHANGE default_rate default_rate DECIMAL(13,3) UNSIGNED NOT NULL");
      	if (is_error($result)) {
      		return $result->getMessage();
      	} // if
      } // if
    ...
    function updateIncomingMailModule() {
      ...
      if(in_array($incoming_mail_activity_logs_table, $this->utility->db->listTables(TABLE_PREFIX))) {
        $result = $this->utility->db->execute("ALTER TABLE $incoming_mail_activity_logs_table 
    ...
    function updateSourceConfigOptions() {
      if(array_var($this->utility->db->execute_one("SELECT COUNT(*) AS 'row_count' FROM " . TABLE_PREFIX . "modules WHERE name = 'source'"), 'row_count') == 1) {
        $this->utility->db->execute("INSERT INTO " . TABLE_PREFIX . "config_options (name, module, type, value) VALUES ('source_svn_use_output_redirect', 'source', 'system', 'b:0;')");
        $this->utility->db->execute("INSERT INTO " . TABLE_PREFIX . "config_options (name, module, type, value) VALUES ('source_svn_trust_server_cert', 'source', 'system', 'b:0;')");
      } // if
      return true;
    ...
    function updateMimeType() {
      ...
      if(in_array(TABLE_PREFIX . 'documents', $this->utility->db->listTables(TABLE_PREFIX))) {
